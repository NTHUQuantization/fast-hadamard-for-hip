...
#include "fast_hadamard_transform_common_to_hip.h"
...
void fast_hadamard_transform_kernel(HadamardParamsBase params) {
    ...
    constexpr int kWarpSize = std::min(kNThreads, 64);
...
void fast_hadamard_transform_cuda(HadamardParamsBase &params, hipStream_t stream) {
    if (params.log_N == 3) {
        ...
        fast_hadamard_transform_launch<1, 4, input_t>(params, stream);
    } else if (params.log_N == 5) {
        fast_hadamard_transform_launch<1, 5, input_t>(params, stream);
    } else if (params.log_N == 6) {
        fast_hadamard_transform_launch<1, 6, input_t>(params, stream);
    } else if (params.log_N == 7) {
        fast_hadamard_transform_launch<1, 7, input_t>(params, stream);
